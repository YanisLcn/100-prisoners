...
def look_for_number(target_id, last_paper, remaining_tries) :
    ...
    if new_paper == target_id :
        return True
    else :
        return look_for_number(target_id, new_paper, remaining_tries -1)
...
def launch_optimal() :
    ...
    for i in range (nb_of_prisoners) :
        if look_for_number(i, i, nb_of_prisoners/2) == False :
            return False
    return True
...
print("Avec la méthode aléatoire, les prisonniers ont survécu "
      +str(cmp_alea)+"/"+str(nb_of_iterations)
...
print("Avec la méthode optimale, les prisonniers ont survécu "
      +str(cmp_opt)+"/"+str(nb_of_iterations)
      +" fois, soit un taux de réussite à hauteur de "
